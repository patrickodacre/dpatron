                    ...
                    <div className="card mb-4" >
                        <div className="card-body">
                            ...
                            <form className="mb-3" onSubmit={(event) => {
                                event.preventDefault()
                                let amount
                                amount = this.unstake_input.value.toString()
                                amount = window.web3.utils.toWei(amount, 'Ether')
                                this.props.unstakeTokens(amount)
                            }}>
                                <div>
                                    <label className="float-left"><b>Unstake mDai Tokens</b></label>
                                    <span className="float-right text-muted">
                                      mDai Staked: {window.web3.utils.fromWei(this.props.stakingBalance, 'Ether')}
                                    </span>
                                </div>
                                <div className="input-group mb-4">
                                    <input
                                        type="text"
                                        ref={(input) => { this.unstake_input = input }}
                                        className="form-control form-control-lg"
                                        placeholder="0"
                                    required />
                                    <div className="input-group-append">
